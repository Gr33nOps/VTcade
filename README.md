...
reimplementing the rules, so they fail when a game actually regresses. All five
...
password, or continue with Google. Confirm your email, then sign in - or skip
all of that with CONTINUE AS GUEST, which drops you straight onto the
dashboard under a throwaway name (`GUESTxxxx`), no account required.
...
make a tiny read against the public leaderboard at most 96 hours apart, which
helps keep database activity flowing through long stretches with no visitors.
GitHub Actions cron cannot express exactly 100 hours, so 96 hours is used to
stay within that limit. It needs two
repository secrets:
